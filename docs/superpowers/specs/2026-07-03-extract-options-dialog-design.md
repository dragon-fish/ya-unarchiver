...
- **默认子文件夹名**:实现取 `hasSingleTopLevelDirectory(entries) ?? archiveBaseName(archive)`(顶层目录名优先),以与旧一键行为逐字节一致;§④ 曾写「包名」。
- **strip 规则**:沿用现有代码语义(任意单顶层目录都去重),未加口述的「与包同名」限定(YAGNI/超范围)。
- **解压位置校验放宽(2026-07-03,用户反馈后)**:§6 原要求「已存在且可写的目录」偏严。改为**允许不存在**,解压时自动创建(贴合 Windows 7-Zip);合法性改判「最近的已存在祖先须为可写目录」,仍拦截穿过文件/落在不可写根等不可能路径。对话框新增「将新建此文件夹」淡蓝提示。commit `82a800a`。
